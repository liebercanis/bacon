...
anaRun::anaRun(TString tag)
{
  ...
  ntupleCal = new TNtuple("ntupleCal","ntupleCal","irun:ientry:baseline0:baseline1:sdev0:sdev1");
  ntupleRun = new TNtuple("ntupleRun","ntupleRun","run:npmt:entry:nhits:nbins:q:start:width:qmax");
  ...
  //for(Int_t irun = irunStart; irun<irunStop +1; irun++){
    ...
    printf(" \t sigma %.2f minLength %i forwardHalfLength %i  backwardLengthTimeThreshold %i \n",
        sigma, minLength,forwardHalfLength, backwardLengthTimeThreshold); 
    printf(" \t  backwardLengthTimeThresholdDouble %.4E forwardLengthTimeThreshold %.4E  deltaT %.3f deltaV %.3f  singlePhoton %.3f \n ",
        backwardLengthTimeThresholdDouble,forwardLengthTimeThreshold , deltaT, deltaV, singlePhoton ); 
    ...
    for (UInt_t ientry=0; ientry<nentries; ientry++) {
      ...
      */
      ...
      //minLength = int(minLength/deltaT);
      ...
      for(int pmtNum = 0 ; pmtNum < NPMT; pmtNum++){
        ...
        if(qSum.size() > 0) {
          ...
          pmtSignalName.Form("PMT_%i_charge_%.2f_peak_%.2f_peakt_%.2f_start_%.2f_width_%.2f_nhit_%i",
              pmtNum, 
              deltaT*qSum[0]*1E10, 
              qhitMax[0]*1E10,
              peakBin[0], 
      ...
      cout<<singlePhoton<<" events with hits out of "<<nentries<<endl;
      //}//irun loop
