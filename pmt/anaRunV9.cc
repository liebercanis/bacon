...
anaRun::anaRun(TString tag, Int_t maxEvents)
{
  ...
  ntBase = new TNtuple("ntBase","base","iw:ks:cs:kw:cw:w:b:bnon:width");
  ...
  for (UInt_t ientry=0; ientry<nentries; ientry++) {
    ...
    if(ientry==0) {
      ...
      for(int ipmt=0; ipmt<gotPMT; ++ipmt) {
        ...
        hBaselineNON[ipmt]  = new TH1D(Form("BaselineNON%i_%s",ipmt,tag.Data()),"",nSamples,pmtXLow,pmtXHigh);
    ...
    for(int pmtNum = 0 ; pmtNum < gotPMT; pmtNum++){
     ...
     // get baseline
      ...
      baselineDigi[pmtNum] = getBaselineWMA(rawAve[pmtNum],ddigi[pmtNum], peakList,unsigned(2*maxHalfLength));
...
std::vector<Double_t> anaRun::getBaselineWMA(Double_t ave, std::vector<Double_t> sig, peakType peakList,Int_t NWindow)
{
  // default baseline is zero
  std::vector<Double_t> baseline(sig.size(),ave);
  if(peakList.size() == 0) return baseline;

  Double_t wnill = 0;
  std::vector<Double_t> weight(sig.size(),wnill);

  //printf(" getBaslineWMA window %u \n",NWindow);
  ...
  for(int ip=0; ip<int(peakList.size()); ++ip) {
    ...
    if(ip==peakList.size()-1) alpha = weight.size();
  ...
  // non recursive sums 
  std::vector<Double_t> basenon(sig.size(),0);
  std::vector<Double_t> kwnon(sig.size(),0);
  std::vector<Double_t> ksnon(sig.size(),0);
  std::vector<Double_t> cwnon(sig.size(),0);
  std::vector<Double_t> csnon(sig.size(),0);
  for(int iw=0; iw<int(sig.size()); ++iw) {
    ...
    hBaselineNON[0]->SetBinContent(iw,basenon[iw]);
  ...
  Double_t SS=0;
  //
  for(int jp =0; jp<= NWindow; ++jp) {
    Double_t cosj = TMath::Cos(  Double_t(jp)*TMath::Pi()/Double_t(NWindow) );
    Double_t sinj = TMath::Sin(  Double_t(jp)*TMath::Pi()/Double_t(NWindow) );
    KW+= weight[jp];
    KS+= sig[jp]*weight[jp];
    CW+= weight[jp]*cosj;
  ...
  baseline[0]=(KS+CS)/(KW+CW);
  printf(" 000000 iw 0 %E (%E)  ratio %.3f  \n",baseline[0],basenon[0],baseline[0]/basenon[0]);

  // recursive sums
  Double_t kcos = TMath::Cos( TMath::Pi()/Double_t(NWindow));
  Double_t ksin = TMath::Cos( TMath::Pi()/Double_t(NWindow));
  printf(" kcos %f ksin %f \n",kcos,ksin);

  for(int iw=1; iw<int(sig.size()); ++iw) {
    hWeight[0]->SetBinContent(iw,weight[iw]);
    ...
    //Double_t diff1 = TMath::Abs(KW+CW);
    //if(diff1<1E-6) printf(" STARTING KS %E CS %E KW %E CW %E base %E \n",KS,CS,KW,CW,baseline[iw]);
      // case sums 
    int i1 = iw+NWindow;
    int i2 = iw-1-NWindow;
    if(iw  <= NWindow && iw + NWindow <= sig.size() - 1){
      KW = KW + weight[i1];
      KS = KS + sig[i1]*weight[i1];
      CW = kcos*CWlast + ksin*SW - weight[i1];
      CS = kcos*CSlast + ksin*SS - sig[i1]*weight[i1];
      SW = kcos*SW  - ksin*CWlast;
      SS = kcos*SS  - ksin*CSlast;
    } else if(iw > NWindow && iw + NWindow <= sig.size() - 1){
      KW = KW - weight[i2] + weight[i1];
      KS = KS - sig[i2]*weight[i1] +  sig[i1]*weight[i1];
      CW = kcos*CWlast+ksin*SW + kcos*weight[i2] - weight[i1];
      CS = kcos*CSlast+ksin*SS + kcos*sig[i2]*weight[i2] - sig[i1]*weight[i1];
      SW = kcos*SW-ksin*CWlast - ksin*weight[i2] ;
      SS = kcos*SS-ksin*CSlast - ksin*sig[i2]*weight[i2] ;
    } else if(iw  > NWindow && iw + NWindow > sig.size() - 1){
      KW = KW - weight[i2];
      KS = KS - sig[i2]*weight[i2];
      CW = kcos*CWlast+ksin*SW + kcos*weight[i2];
      CS = kcos*CSlast+ksin*SS + kcos*sig[i2]*weight[i2];
      SW = kcos*SW-ksin*CWlast - ksin*weight[i2] ;
      SS = kcos*SS-ksin*CSlast - ksin*sig[i2]*weight[i2] ;
    } else if(iw <= NWindow && iw + NWindow > sig.size() - 1){
      KW = KW;
    ...
    // sum baseline 
    if( TMath::Abs(KW+CW)>1E-9) baseline[iw]=(KS+CS)/(KW+CW);
    if( TMath::Abs(baseline[iw]/basenon[iw]) > 10.0  ) baseline[iw]=basenon[iw]; 
    ntBase->Fill(float(iw),KS,CS,KW,CW,weight[iw],baseline[iw],basenon[iw],float(maxwidth));
    
    if(iw<10 ) {
      printf(" XXXXXX iw %u  weight %.1f (%u,%u) %E (%E)  ratio %.3f  \n",iw,weight[iw],low,high,baseline[iw],basenon[iw],baseline[iw]/basenon[iw]);
      printf("        kw %E (%E) ks %E (%E) cw %E (%E) cs %E (%E) \n",KW,kwnon[iw],KS,ksnon[iw],CW,cwnon[iw],CS,csnon[iw]);
    } else if( TMath::Abs(baseline[iw]/basenon[iw]) > 10.0  ) { 
     printf(" XXXXXX iw %u   weight %.1f (%u,%u) %E (%E) ratio %.3f \n",iw,weight[iw],low,high,baseline[iw],basenon[iw],baseline[iw]/basenon[iw]);
    }
