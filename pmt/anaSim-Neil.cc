...
anaSim::anaSim(){
  ...
  TString fileDir = "/home/nmcfadden/RooT/PMT/rootData/";
  for(int z = 152; z <= 158; z++){
  if(z == 10) continue;
  if(z == 26) continue;
  ...
  TString outFileName = TString("SimAnaResults.")+fileName;
...
//    cout<<"finished filling ntuple"<<endl;
    ...
    signal.clear();
    derivative.clear();
    if(ientry == nentries -1) eventInfo->Write();
