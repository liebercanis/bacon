...
genPulses::genPulses(Int_t maxEvents)
{
  ...
  // histograms
  outFile->cd(); 
  hTest = new TH1D("GenTime","generated lifetime ",1000,startTime*1E6,stopTime*1E6);
  hTestq = new TH1D("Genq","generated charge",1000,startTime*1E6,stopTime*1E6);
...
std::vector<Double_t> genPulses::PulseStartTime(Int_t event, Int_t nPhotons, Double_t tau3)
{
  ...
  while(counter < nPhotons){
    failCounter++;
    if(failCounter > 1e7) { printf(" fail counter maxed out at %i\n",failCounter);  break;}
    Double_t x = startTime - tau3*TMath::Log(1-rand.Rndm());
